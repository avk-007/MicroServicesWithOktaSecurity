...
class 2:first created user.
 ...
 project(service registry) userService,hotelService,ratingService

 class 4: Lets Start Creating UserMicroservice
 dependency-lombok,web,jpa,mysql
 step1:entity-user,later we added ratingEntity in userService by marking it with
 @transient private list<rating>ratings=new Array<>;
 step 2.added exceptionhandling
 step3:.yml with mysql
 ...
 steps:: entity hotel,created global exception class & resourceNotFoundExcetion.
 step 2: application.yml config with postgresql.


 class 7:Complete Rating Microservice with MongoDB in one shot(create project)

  step1:entity rating and add add rating in userService entity too.
  step2: no hibernate jpa required for mongoDb,lombok
  step3: we use @document instead of @Entity.

 class 8:create Service Registry using Eureka SERVER

 step1:(create new project)
 step2: .yml port:8761
 step3: @EnableEurekaServer in main class
 step3: Add this
 ((by implementing DiscoveryServer netflix eureka Server))
 :: with dependency ::(netflix eureka Server(eureka server and spring cloud discovery),spring cloud)
        ...
        important to use serviceRegistry in all the services add @EnableEurekaClient in the main class of
        userService,RatingService and HotelService
       ------------------------------------------------------------------------------
 ...
 step 2:.yml configuration
  &&&& to work ((serviceRegistry)) in userService we copy
      ...
      important add @EnableEurekaClient in the main class of userService,RatingService and HotelService

 class 10:Implementing Service Discovery Client in Hotel Microservice

 same process for both the hotel and rating
 class 11:class 10:Implementing Service Discovery Client in rating Microservice
